...
class ExprNode {
 public:
  ...
  static double eval(const std::shared_ptr<ExprNode>& n,
    ...
    switch (n->kind) {
      case ExprKind::Literal:  return n->value;
      case ExprKind::ParamRef: return params[n->param_idx];
      case ExprKind::Neg:      return -eval(n->lhs, params);
      case ExprKind::Add: return eval(n->lhs, params) + eval(n->rhs, params);
      case ExprKind::Sub: return eval(n->lhs, params) - eval(n->rhs, params);
      case ExprKind::Mul: return eval(n->lhs, params) * eval(n->rhs, params);
      case ExprKind::Div: {
      ...
      case ExprKind::Lt:  return eval(n->lhs, params) <  eval(n->rhs, params) ?
                                 1.0 : 0.0;
      case ExprKind::Gt:  return eval(n->lhs, params) >  eval(n->rhs, params) ?
                          1.0 : 0.0;
      case ExprKind::Le:  return eval(n->lhs, params) <= eval(n->rhs, params) ?
                          1.0 : 0.0;
      case ExprKind::Ge:  return eval(n->lhs, params) >= eval(n->rhs, params) ?
                          1.0 : 0.0;
      case ExprKind::Eq:  return eval(n->lhs, params) == eval(n->rhs, params) ?
                          1.0 : 0.0;
      case ExprKind::Ne:  return eval(n->lhs, params) != eval(n->rhs, params) ?
                          1.0 : 0.0;
      ...
      case ExprKind::LogOr:
        return (eval(n->lhs, params) != 0.0 || eval(n->rhs, params) != 0.0) ?
                1.0 : 0.0;
      case ExprKind::Sin:    return std::sin(  eval(n->lhs, params));
      case ExprKind::Cos:    return std::cos(  eval(n->lhs, params));
      case ExprKind::Tan:    return std::tan(  eval(n->lhs, params));
      case ExprKind::Exp:    return std::exp(  eval(n->lhs, params));
      case ExprKind::Sqrt:   return std::sqrt( eval(n->lhs, params));
      case ExprKind::Ln:     return std::log(  eval(n->lhs, params));
      case ExprKind::Arcsin: return std::asin( eval(n->lhs, params));
      case ExprKind::Arccos: return std::acos( eval(n->lhs, params));
      case ExprKind::Arctan: return std::atan( eval(n->lhs, params));
      case ExprKind::Floor:  return std::floor(eval(n->lhs, params));
    }
...
double parseTimingLiteral(const std::string& text, double dt_ns) {
  ...
  if (unit == "s")  return num * 1e9;
  if (unit == "dt") return num * dt_ns;
...
// ---------------------------------------------------------------------------
static std::uint8_t extractIntLiteral(qasm3Parser::ExpressionContext* expr,
                                       std::uint8_t default_val = 1) {
  auto* lit = dynamic_cast<qasm3Parser::LiteralExpressionContext*>(expr);
  if (lit && lit->DecimalIntegerLiteral())
...
static int modifierCount(qasm3Parser::GateModifierContext* mod) {
...
static Mat identityMatrix(std::uint8_t n) {
...
static Mat conjugateTranspose(const Mat& m, std::uint8_t n) {
  const std::size_t dim = std::size_t{1} << n;
  Mat result(dim * dim);
...
static Mat matMul(const Mat& a, const Mat& b, std::size_t dim) {
...
static Mat matrixPow(const Mat& m, std::uint8_t n, int power) {
  const std::size_t dim = std::size_t{1} << n;
  Mat result = identityMatrix(n);
  for (int p = 0; p < power; ++p) result = matMul(result, m, dim);
  return result;
}

// Prepends one control qubit: identity on |0⟩, U on |1⟩.
static Mat addControl(const Mat& m, std::uint8_t n) {
  const std::size_t dim     = std::size_t{1} << n;
...
static Mat addNegControl(const Mat& m, std::uint8_t n) {
  const std::size_t dim     = std::size_t{1} << n;
  const std::size_t new_dim = dim * 2;
  Mat result(new_dim * new_dim, C{0.0, 0.0});
  for (std::size_t i = 0; i < dim; ++i)
...
static Mat embedGate(const Mat& gate_mat, std::uint8_t gate_n,
                     const std::vector<std::uint8_t>& qubit_indices,
                     std::uint8_t total_n) {
  if (gate_n == total_n) return gate_mat;
  ...
  for (std::size_t in_state = 0; in_state < total_dim; ++in_state) {
    ...
    for (std::size_t gate_out = 0; gate_out < gate_dim; ++gate_out) {
      ...
      for (std::uint8_t k = 0; k < gate_n; ++k) {
        std::uint8_t bit = total_n - 1 - qubit_indices[k];
        if (gate_out & (std::size_t{1} << (gate_n - 1 - k)))
...
class CircuitBuilder : public qasm3ParserBaseVisitor {
 public:
  ...
  std::any visitAssignmentStatement(
    ...
    if (auto* meas = ctx->measureExpression()) {
      ...
      if (checkUndefined(UndefinedKind::BitRegister, lhs_name, ctx))
        return visitChildren(ctx);
    ...
    if (!ctx->expression()) return visitChildren(ctx);

    auto* lhs = ctx->indexedIdentifier();
    const std::string name = lhs->Identifier()->getText();
    if (checkConstReassignment(name, ctx)) return visitChildren(ctx);
    ...
    if (!existing) {
      addError(errUndefined(UndefinedKind::Variable, name), ctx);
      return visitChildren(ctx);
    }
    ...
    const double rhs =
        ExprNode::eval(buildExprTree(ctx->expression()), {});
    double cur = existing->real();

    if      (op == "+=")  cur += rhs;
    else if (op == "-=")  cur -= rhs;
    else if (op == "*=")  cur *= rhs;
    else if (op == "/=")  cur  = (rhs != 0.0) ? cur / rhs : 0.0;
    else if (op == "%=")  cur  = std::fmod(cur, rhs);
    else if (op == "**=") cur  = std::pow(cur, rhs);
    else if (op == "&=")  {
    ...
    if (!ctx->gateOperandList()) {
      ...
      for (std::size_t r = 0; r < qubit_registers_.size(); ++r)
        for (std::size_t q = 0; q < qubit_registers_[r].size; ++q)
          qubits.push_back({static_cast<std::uint8_t>(r),
      ...
      for (auto* op : ctx->gateOperandList()->gateOperand()) {
        ...
        if (indexed && indexed->indexOperator().empty()) {
  ...
  std::any visitClassicalDeclarationStatement(
    ...
    if (checkRedeclared(name, ctx)) return visitChildren(ctx);

    // Array type: store elements as "name[i]", "name[i][j]", etc.
    if (ctx->arrayType()) {
      if (auto* decl = ctx->declarationExpression()) {
        if (auto* cast =
            dynamic_cast<qasm3Parser::CastExpressionContext*>(
                decl->expression())) {
          ...
          if (cast->arrayType()) {
            if (auto* src_lit =
                dynamic_cast<qasm3Parser::LiteralExpressionContext*>(
                    cast->expression())) {
              if (src_lit->Identifier()) {
                const std::string src =
                    src_lit->Identifier()->getText();
    ...
    if (!scalar) return visitChildren(ctx);

    if (scalar->BIT()) {
      if (bit_map_.count(name) || qubit_map_.count(name)) {
        addError("redeclaration of '" + name + "'", ctx);
        return visitChildren(ctx);
      }
      const std::size_t size = scalar->designator()
    ...
    } else if (scalar->FLOAT() || scalar->INT() || scalar->UINT() ||
               scalar->ANGLE()) {
    ...
    } else if (scalar->DURATION()) {
      setScalar(name, 0.0);
      if (ctx->declarationExpression())
        if (auto* expr = ctx->declarationExpression()->expression())
          setScalar(name, ExprNode::eval(buildExprTree(expr), {}));
    } else if (scalar->STRETCH()) {
      setScalar(name, 0.0);
    }
    return visitChildren(ctx);
  }
  ...
  std::any visitGateCallStatement(
    ...
    if (ctx->GPHASE()) {
      ...
      if (modifiers.empty()) return visitChildren(ctx);
      ...
      if (!gate) return visitChildren(ctx);
      ...
      if (qubits.size() != gate->numQubits()) {
        addError("ctrl@gphase expects " + std::to_string(gate->numQubits()) +
                 " qubit(s), got " + std::to_string(qubits.size()), ctx);
        return visitChildren(ctx);
      }
      operations_.push_back(
    ...
    if (checkUndefined(UndefinedKind::Gate, gate_name, ctx))
      return visitChildren(ctx);
    ...
    if (!operand_list) {
      addError("gate '" + gate_name + "' called with no qubit operands",
               ctx);
      return visitChildren(ctx);
    }
    ...
    if (gate->numQubits() == 1 && modifiers.empty() &&
        operands.size() == 1) {
      ...
      if (indexed && indexed->indexOperator().empty()) {
        ...
        if (it != qubit_map_.end()) {
          for (std::size_t q = 0; q < it->second.size; ++q)
            operations_.push_back(
                {OperationType::Gate, gate, params,
                 {{static_cast<std::uint8_t>(it->second.index),
                   static_cast<std::uint8_t>(q)}},
                 {}});
          return visitChildren(ctx);
    ...
    if (modifiers.empty() &&
      ...
      for (auto* op : operands) {
        ...
        if (!idx || !idx->indexOperator().empty()) {
          all_whole = false; break;
        }
        auto it = qubit_map_.find(idx->Identifier()->getText());
        if (it == qubit_map_.end()) { all_whole = false; break; }
        regs.push_back(&it->second);
      }
      if (all_whole && regs.size() > 1) {
        const std::size_t sz = regs[0]->size;
        bool same = true;
        for (auto* r : regs) if (r->size != sz) { same = false; break; }
        if (same) {
          for (std::size_t q = 0; q < sz; ++q) {
            std::vector<QubitReference> qubits;
            for (auto* r : regs)
              qubits.push_back(
                  {static_cast<std::uint8_t>(r->index),
                   static_cast<std::uint8_t>(q)});
            operations_.push_back(
                {OperationType::Gate, gate, params,
                 std::move(qubits), {}});
    ...
    if (qubits.size() != gate->numQubits()) {
      addError(errWrongQubitCount(gate->name(),
               static_cast<int>(gate->numQubits()),
               static_cast<int>(qubits.size())), ctx);
      return visitChildren(ctx);
    }
    operations_.push_back({OperationType::Gate, gate, std::move(params),
                            std::move(qubits), {}});
    return visitChildren(ctx);
  }
  ...
  std::any visitGateStatement(
    ...
    const std::string name = ctx->Identifier()->getText();
    if (checkRedefined(RedefinedKind::Gate, name, ctx)) return {};
    ...
    struct BodyMod {
      enum class Kind { Inv, Pow, Ctrl, NegCtrl };
      Kind kind;
      int  count = 1;
    };
    struct BodyCall {
    ...
    for (auto* sos : ctx->scope()->statementOrScope()) {
      ...
      if (!body_mods.empty() && call_gate->numParams() == 0) {
        ...
        if (!call_gate) continue;
        bc.gate = call_gate;
      } else if (!body_mods.empty()) {
        // Parametric base: record modifiers, apply at call time
        for (auto* mod : body_mods) {
          if (mod->INV()) {
            bc.modifiers.push_back({BodyMod::Kind::Inv, 1});
          } else if (mod->POW()) {
            bc.modifiers.push_back(
                {BodyMod::Kind::Pow, modifierCount(mod)});
          } else if (mod->CTRL()) {
            bc.modifiers.push_back(
                {BodyMod::Kind::Ctrl, modifierCount(mod)});
          } else if (mod->NEGCTRL()) {
            bc.modifiers.push_back(
                {BodyMod::Kind::NegCtrl, modifierCount(mod)});
          }
        }
      ...
      registry_.add(GateDefinition(
          ...
          [body, num_qubits](const std::vector<double>& params) -> Mat {
            ...
            for (const auto& bc : body) {
              ...
              for (int i = static_cast<int>(bc.modifiers.size()) - 1;
                   i >= 0; --i) {
                const auto& mod = bc.modifiers[i];
                switch (mod.kind) {
                  case BodyMod::Kind::Inv:
                    gate_mat = conjugateTranspose(gate_mat, gate_n);
                    break;
                  case BodyMod::Kind::Pow:
                    gate_mat = matrixPow(gate_mat, gate_n, mod.count);
                    break;
                  case BodyMod::Kind::Ctrl:
                    for (int c = 0; c < mod.count; ++c)
                      { gate_mat = addControl(gate_mat, gate_n); ++gate_n; }
                    break;
                  case BodyMod::Kind::NegCtrl:
                    for (int c = 0; c < mod.count; ++c)
                      { gate_mat = addNegControl(gate_mat, gate_n); ++gate_n; }
                    break;
                }
              }
              Mat embedded = embedGate(gate_mat, gate_n,
                                       bc.qubit_indices, num_qubits);
              result = matMul(embedded, result, dim);
            }
            return result;
  ...
  std::any visitMeasureArrowAssignmentStatement(
    ...
    if (!meas) return visitChildren(ctx);
    ...
    if (q_indexed) {
      ...
      if (!qubit_map_.count(qname)) {
        addError(errUndefined(UndefinedKind::QubitRegister, qname), ctx);
        return visitChildren(ctx);
      }
    }
    if (bit_target) {
      const std::string cname = bit_target->Identifier()->getText();
      if (!bit_map_.count(cname)) {
        addError(errUndefined(UndefinedKind::BitRegister, cname), ctx);
        return visitChildren(ctx);
      }
    }

    // Whole-register broadcast: measure q -> c expands to N per-qubit ops
    if (q_indexed && q_indexed->indexOperator().empty() &&
        bit_target  && bit_target->indexOperator().empty()) {
      ...
      if (q_it != qubit_map_.end() && c_it != bit_map_.end()) {
        if (q_it->second.size != c_it->second.size) {
          addError(
              "register size mismatch in measurement: '" +
              q_it->first + "' (" +
              std::to_string(q_it->second.size) + " qubits) vs '" +
              c_it->first + "' (" +
              std::to_string(c_it->second.size) + " bits)", ctx);
          return visitChildren(ctx);
        }
        for (std::size_t i = 0; i < q_it->second.size; ++i)
          operations_.push_back(
              {OperationType::Measure, nullptr, {},
    ...
    if (bit_target) targets.push_back(resolveBit(bit_target, ctx));
    operations_.push_back(
  ...
  std::any visitOldStyleDeclarationStatement(
    ...
    const std::string name = ctx->Identifier()->getText();
    if (checkRedeclared(name, ctx)) return visitChildren(ctx);
    const std::size_t size = ctx->designator()
        ? static_cast<std::size_t>(ExprNode::eval(
  ...
  std::any visitQuantumDeclarationStatement(
    ...
    const std::string name = ctx->Identifier()->getText();
    if (checkRedeclared(name, ctx)) return visitChildren(ctx);
    const std::size_t size = ctx->qubitType()->designator()
        ? static_cast<std::size_t>(ExprNode::eval(
              buildExprTree(
  ...
  std::any visitResetStatement(
    ...
    if (indexed && indexed->indexOperator().empty()) {
      ...
      if (it != qubit_map_.end()) {
        for (std::size_t q = 0; q < it->second.size; ++q)
          operations_.push_back(
              {OperationType::Reset, nullptr, {},
               {{static_cast<std::uint8_t>(it->second.index),
                 static_cast<std::uint8_t>(q)}},
               {}});
        return visitChildren(ctx);
      }
      checkUndefined(UndefinedKind::QubitRegister, name, ctx);
      return visitChildren(ctx);
    }
 ...
 private:
  ...
  enum class UndefinedKind  { Gate, Subroutine, QubitRegister, BitRegister,
                              Variable };
  enum class RedeclaredKind { QubitRegister, BitRegister, Variable };
  enum class RedefinedKind  { Gate, Subroutine };
  ...
  std::unordered_map<std::string,
      std::shared_ptr<GateDefinition>>            derived_gates_;
  ...
  static std::string errUndefined(UndefinedKind kind,
                                  const std::string& name) {
    switch (kind) {
      case UndefinedKind::Gate:          
        return "undefined gate '" + name + "'";
      case UndefinedKind::Subroutine:
        return "undefined subroutine '" + name + "'";
      case UndefinedKind::QubitRegister:
        return "undefined qubit register '" + name + "'";
      case UndefinedKind::BitRegister:
        return "undefined bit register '" + name + "'";
      case UndefinedKind::Variable:
  ...
  static std::string errRedeclared(RedeclaredKind kind,
                                   const std::string& name) {
    switch (kind) {
      case RedeclaredKind::QubitRegister:
        return "redeclaration of qubit register '" + name + "'";
      case RedeclaredKind::BitRegister:
        return "redeclaration of bit register '" + name + "'";
      case RedeclaredKind::Variable:
  ...
  static std::string errRedefined(RedefinedKind kind,
                                  const std::string& name) {
    switch (kind) {
      case RedefinedKind::Gate:
        return "redefinition of gate '" + name + "'";
      case RedefinedKind::Subroutine:
  ...
  static std::string errWrongQubitCount(const std::string& name,
  ...
  const C* lookupScalar(const std::string& name) const {
    for (auto it = scopes_.rbegin(); it != scopes_.rend(); ++it) {
      auto found = it->scalar_map.find(name);
      if (found != it->scalar_map.end()) return &found->second;
    }
    return nullptr;
  }
  ...
  bool isConst(const std::string& name) const {
    for (auto it = scopes_.rbegin(); it != scopes_.rend(); ++it)
      if (it->const_names.count(name)) return true;
    return false;
  }

  bool checkRedeclared(const std::string& name,
                       antlr4::ParserRuleContext* ctx) {
    if (qubit_map_.count(name)) {
      addError(errRedeclared(RedeclaredKind::QubitRegister, name), ctx);
      return true;
    }
    if (bit_map_.count(name)) {
      addError(errRedeclared(RedeclaredKind::BitRegister, name), ctx);
      return true;
    }
    if (lookupScalar(name) != nullptr) {
      addError(errRedeclared(RedeclaredKind::Variable, name), ctx);
      return true;
    }
    return false;
  }
  ...
  bool checkUndefined(UndefinedKind kind, const std::string& name,
    ...
    switch (kind) {
      case UndefinedKind::Gate:
        is_undefined = !registry_.contains(name); break;
      case UndefinedKind::Subroutine:
        is_undefined = !subroutine_map_.count(name); break;
      case UndefinedKind::QubitRegister:
        is_undefined = !qubit_map_.count(name); break;
      case UndefinedKind::BitRegister:
        is_undefined = !bit_map_.count(name); break;
      case UndefinedKind::Variable:
        is_undefined = (lookupScalar(name) == nullptr); break;
    }
  ...
  bool checkRedefined(RedefinedKind kind, const std::string& name,
    ...
    switch (kind) {
      case RedefinedKind::Gate:
        is_redefined = registry_.contains(name); break;
      case RedefinedKind::Subroutine:
        is_redefined = subroutine_map_.count(name); break;
    }

    if (!is_redefined) return false;
  ...
  QubitReference resolveQubit(qasm3Parser::GateOperandContext* operand,
    ...
    auto it = qubit_map_.find(name);
    if (it == qubit_map_.end()) {
      addError(errUndefined(UndefinedKind::QubitRegister, name), error_ctx);
      return {0, 0};
    }
    ...
    if (!indexed->indexOperator().empty()) {
      ...
      if (!idx_op->expression().empty())
        qubit_idx = static_cast<std::size_t>(ExprNode::eval(
            buildExprTree(idx_op->expression(0)), {}));
    }
  ...
  BitReference resolveBit(qasm3Parser::IndexedIdentifierContext* indexed,
    ...
    if (it == bit_map_.end()) {
      addError(errUndefined(UndefinedKind::BitRegister, name), error_ctx);
      return {0, 0};
    }
    ...
    if (!indexed->indexOperator().empty()) {
      auto* idx_op = indexed->indexOperator(0);
      if (!idx_op->expression().empty())
        bit_idx = static_cast<std::size_t>(ExprNode::eval(
            buildExprTree(idx_op->expression(0)), {}));
    }
    return {static_cast<std::uint8_t>(it->second.index),
  ...
  static std::string buildModifiedName(
    ...
    for (auto* mod : modifiers) {
      if (mod->INV()) {
        ...
        for (int i = 0; i < modifierCount(mod); ++i) name += "ctrl@";
      } else if (mod->NEGCTRL()) {
        for (int i = 0; i < modifierCount(mod); ++i) name += "negctrl@";
      }
    }
    name += base_name;
  ...
  std::shared_ptr<const GateDefinition> deriveModifiedGate(
      std::shared_ptr<const GateDefinition> base,
      const std::vector<double>& base_params,
      const std::vector<qasm3Parser::GateModifierContext*>& modifiers,
      antlr4::ParserRuleContext* error_ctx) {
    const std::string name =
    ...
    if (it != derived_gates_.end()) return it->second;
    ...
    for (int i = static_cast<int>(modifiers.size()) - 1; i >= 0; --i) {
      ...
      if (mod->INV()) {
        ...
        for (int c = 0; c < count; ++c) { mat = addControl(mat, n); ++n; }
      } else if (mod->NEGCTRL()) {
        const int count = modifierCount(mod);
        for (int c = 0; c < count; ++c) { mat = addNegControl(mat, n); ++n; }
      } else {
  ...
  std::shared_ptr<const GateDefinition> deriveGphaseWithModifiers(
    ...
    for (int i = static_cast<int>(modifiers.size()) - 1; i >= 0; --i) {
      ...
      if (mod->INV()) {
        ...
        C result{1.0, 0.0};
        for (int p = 0; p < modifierCount(mod); ++p) result *= mat[0];
        mat[0] = result;
      } else if (mod->CTRL()) {
        const int count = modifierCount(mod);
        for (int c = 0; c < count; ++c) { mat = addControl(mat, n); ++n; }
    ...
    if (n == 0) return nullptr;  // no qubit modifiers — unobservable
    auto gate = std::make_shared<GateDefinition>(name, n, std::move(mat));
    derived_gates_[name] = gate;
    return gate;
  }
  ...
  ExprPtr buildExprTreeImpl(
    ...
    const double dt_ns = config_.dtNs();
    ...
    // expression indexOperator
        ...
        dynamic_cast<qasm3Parser::IndexExpressionContext*>(expr)) {
      std::vector<int> indices;
      std::string arr_name;
      auto* cur = e;
      while (cur) {
        if (!cur->indexOperator()) break;
        ...
        for (size_t i = 0; i < idxOp->expression().size(); ++i) {
          auto idx_tree = buildExprTreeImpl(
              idxOp->expression(i), param_idx, const_vals);
        ...
        if (auto* lit =
          ...
          if (lit->Identifier()) arr_name = lit->Identifier()->getText();
          break;
        } else if (auto* next =
      ...
      if (!arr_name.empty()) {
        ...
        if (!declared) {
          addError(errUndefined(UndefinedKind::Variable, arr_name), expr);
        } else {
          auto it = const_vals.find(key);
          if (it != const_vals.end()) { n->value = it->second; return n; }
    ...
    // op=(TILDE | EXCLAMATION_POINT | MINUS) expression
      ...
      if (e->TILDE())                  n->kind = ExprNode::ExprKind::BitNot;
      else if (e->EXCLAMATION_POINT()) n->kind = ExprNode::ExprKind::LogNot;
      else if (e->MINUS())             n->kind = ExprNode::ExprKind::Neg;
      else assert(false && "unhandled operator in UnaryExpressionContext");
    ...
    // expression op=(ASTERISK | SLASH | PERCENT) expression
      ...
      if (e->ASTERISK())     n->kind = ExprNode::ExprKind::Mul;
      else if (e->SLASH())   n->kind = ExprNode::ExprKind::Div;
      else if (e->PERCENT()) n->kind = ExprNode::ExprKind::Mod;
      else assert(false &&
    ...
    // expression op=(PLUS | MINUS) expression
      ...
      if (e->PLUS())       n->kind = ExprNode::ExprKind::Add;
      else if (e->MINUS()) n->kind = ExprNode::ExprKind::Sub;
      else assert(false && "unhandled operator in AdditiveExpressionContext");
      n->lhs = buildExprTreeImpl(e->expression(0), param_idx, const_vals);
      n->rhs = buildExprTreeImpl(e->expression(1), param_idx, const_vals);
    ...
    // expression op=BitshiftOperator expression
      ...
      if (e->op->getText() == "<<")      n->kind = ExprNode::ExprKind::LShift;
      else if (e->op->getText() == ">>") n->kind = ExprNode::ExprKind::RShift;
      else assert(false && "unhandled operator in BitshiftExpressionContext");
    ...
    // expression op=ComparisonOperator expression
      ...
      if (e->op->getText() == ">")      n->kind = ExprNode::ExprKind::Gt;
      else if(e->op->getText() == "<")  n->kind = ExprNode::ExprKind::Lt;
      else if(e->op->getText() == ">=") n->kind = ExprNode::ExprKind::Ge;
      else if(e->op->getText() == "<=") n->kind = ExprNode::ExprKind::Le;
      else assert(false && "unhandled operator in ComparisonExpressionContext");
    ...
    // expression op=EqualityOperator expression
      ...
      if (e->op->getText() == "==")      n->kind = ExprNode::ExprKind::Eq;
      else if(e->op->getText() == "!=")  n->kind = ExprNode::ExprKind::Ne;
      else assert(false && "unhandled operator in EqualityExpressionContext");
    ...
    // expression op=AMPERSAND expression
      ...
      n->kind = ExprNode::ExprKind::BitAnd;
      n->lhs  = buildExprTreeImpl(e->expression(0), param_idx, const_vals);
      n->rhs  = buildExprTreeImpl(e->expression(1), param_idx, const_vals);
    ...
    // expression op=CARET expression
      ...
      n->kind = ExprNode::ExprKind::BitXor;
      n->lhs  = buildExprTreeImpl(e->expression(0), param_idx, const_vals);
      n->rhs  = buildExprTreeImpl(e->expression(1), param_idx, const_vals);
    ...
    // expression op=PIPE expression
      ...
      n->kind = ExprNode::ExprKind::BitOr;
      n->lhs  = buildExprTreeImpl(e->expression(0), param_idx, const_vals);
      n->rhs  = buildExprTreeImpl(e->expression(1), param_idx, const_vals);
    ...
    // expression op=DOUBLE_AMPERSAND expression
      ...
      n->kind = ExprNode::ExprKind::LogAnd;
      n->lhs  = buildExprTreeImpl(e->expression(0), param_idx, const_vals);
      n->rhs  = buildExprTreeImpl(e->expression(1), param_idx, const_vals);
    ...
    // expression op=DOUBLE_PIPE expression
      ...
      n->kind = ExprNode::ExprKind::LogOr;
      n->lhs  = buildExprTreeImpl(e->expression(0), param_idx, const_vals);
      n->rhs  = buildExprTreeImpl(e->expression(1), param_idx, const_vals);
    ...
    // (scalarType | arrayType) LPAREN expression RPAREN
      ...
      if (e->scalarType()) {
        ...
        if (e->scalarType()->INT() || e->scalarType()->UINT()) {
          auto inner = n;
          n = std::make_shared<ExprNode>();
          n->kind = ExprNode::ExprKind::Floor;
          n->lhs  = inner;
        }
      } else {
    ...
    // DURATIONOF LPAREN scope RPAREN
               ...
               dynamic_cast<qasm3Parser::DurationofExpressionContext*>(expr)) {
    ...
    // Identifier LPAREN expressionList? RPAREN
      ...
      {
        {"sin",    ExprNode::ExprKind::Sin},
        {"cos",    ExprNode::ExprKind::Cos},
        {"tan",    ExprNode::ExprKind::Tan},
        {"exp",    ExprNode::ExprKind::Exp},
        {"sqrt",   ExprNode::ExprKind::Sqrt},
        {"ln",     ExprNode::ExprKind::Ln},
        {"arcsin", ExprNode::ExprKind::Arcsin},
        {"arccos", ExprNode::ExprKind::Arccos},
        {"arctan", ExprNode::ExprKind::Arctan},
      };
      auto it = kFnMap.find(e->Identifier()->getText());
      if (it != kFnMap.end() && e->expressionList() &&
          !e->expressionList()->expression().empty()) {
        n->kind = it->second;
        n->lhs  = buildExprTreeImpl(
            e->expressionList()->expression(0), param_idx, const_vals);
      } else {
    ...
    // TimingLiteral | HardwareQubit
      ...
      if (e->DecimalIntegerLiteral()) {
        ...
        if (pit != param_idx.end()) {
          n->kind      = ExprNode::ExprKind::ParamRef;
          n->param_idx = pit->second;
        } else {
          auto cit = const_vals.find(id);
          if (cit != const_vals.end()) { n->value = cit->second; }
          else if (id == "pi")    { n->value = kPi; }
          else if (id == "tau")   { n->value = kTau; }
          else if (id == "euler") { n->value = kEuler; }
          else addError(errUndefined(UndefinedKind::Variable, id), expr);
        }
      } else {
  ...
  C evalC(qasm3Parser::ExpressionContext* expr) {
    if (auto* e =
      ...
      if (e->Identifier()) {
        ...
        if (const C* v = lookupScalar(id)) return *v;
        if (id == "pi")    return {kPi,    0.0};
        if (id == "tau")   return {kTau,   0.0};
        if (id == "euler") return {kEuler, 0.0};
        return {0.0, 0.0};
      }
    ...
    if (auto* e =
        dynamic_cast<qasm3Parser::UnaryExpressionContext*>(expr)) {
      if (e->MINUS()) return -evalC(e->expression());
      return evalC(e->expression());
    }
    if (auto* e =
        dynamic_cast<qasm3Parser::AdditiveExpressionContext*>(expr)) {
      ...
      return e->PLUS() ? l + r : l - r;
    }
    if (auto* e =
        dynamic_cast<qasm3Parser::MultiplicativeExpressionContext*>(expr)) {
      const C l = evalC(e->expression(0));
      const C r = evalC(e->expression(1));
      if (e->ASTERISK()) return l * r;
      if (e->SLASH())    return r != C{0.0, 0.0} ? l / r : C{0.0, 0.0};
    ...
    if (auto* e =
      ...
      if (e->expressionList() &&
        ...
        if (fn == "arcsin") return std::asin(arg);
        if (fn == "arccos") return std::acos(arg);
        if (fn == "arctan") return std::atan(arg);
      }
    }
    return {ExprNode::eval(buildExprTree(expr), {}), 0.0};
  }
