...
static Mat addControl(const Mat& m, std::uint8_t n) {
  ...
  for (std::size_t i = 0; i < dim; ++i) result[i * new_dim + i] = C{1.0, 0.0};
  for (std::size_t i = 0; i < dim; ++i)
    for (std::size_t j = 0; j < dim; ++j)
...
class CircuitBuilder : public qasm3ParserBaseVisitor {
 public:
  ...
  std::any visitGateCallStatement(
    ...
    for (auto* operand : operands) qubits.push_back(resolveQubit(operand, ctx));
      ...
      if (!body_mods.empty() && call_gate->numParams() == 0) {
        ...
        for (auto* mod : body_mods) {
          if (mod->INV()) {
            ...
            bc.modifiers.push_back({BodyMod::Kind::Pow, modifierCount(mod)});
          } else if (mod->CTRL()) {
            bc.modifiers.push_back({BodyMod::Kind::Ctrl, modifierCount(mod)});
          } else if (mod->NEGCTRL()) {
  ...
  std::any visitMeasureArrowAssignmentStatement(
    ...
    if (q_indexed && q_indexed->indexOperator().empty() && bit_target &&
        bit_target->indexOperator().empty()) {
 ...
 private:
  ...
  bool checkUndefined(UndefinedKind kind, const std::string& name,
    ...
    switch (kind) {
      case UndefinedKind::Gate:
        ...
        break;
      case UndefinedKind::Subroutine:
        is_undefined = !subroutine_map_.count(name);
        break;
      case UndefinedKind::QubitRegister:
        is_undefined = !qubit_map_.count(name);
        break;
      case UndefinedKind::BitRegister:
        is_undefined = !bit_map_.count(name);
        break;
      case UndefinedKind::Variable:
  ...
  bool checkRedefined(RedefinedKind kind, const std::string& name,
    ...
    switch (kind) {
      case RedefinedKind::Gate:
        ...
        break;
      case RedefinedKind::Subroutine:
        is_redefined = subroutine_map_.count(name);
        break;
    }
  ...
  ExprPtr buildExprTreeImpl(
    ...
    if (auto* e =
      ...
      while (cur) {
        ...
        if (auto* lit =
          ...
          break;
                       ...
                       dynamic_cast<qasm3Parser::IndexExpressionContext*>(
                           base)) {
          cur = next;
        } else {
      ...
      if (e->TILDE())
        n->kind = ExprNode::ExprKind::BitNot;
      else if (e->EXCLAMATION_POINT())
        n->kind = ExprNode::ExprKind::LogNot;
      else if (e->MINUS())
        n->kind = ExprNode::ExprKind::Neg;
      else
      ...
      // expression op=(ASTERISK | SLASH | PERCENT) expression
                   ...
                   dynamic_cast<qasm3Parser::MultiplicativeExpressionContext*>(
                       expr)) {
      if (e->ASTERISK())
        n->kind = ExprNode::ExprKind::Mul;
      else if (e->SLASH())
        n->kind = ExprNode::ExprKind::Div;
      else if (e->PERCENT())
        n->kind = ExprNode::ExprKind::Mod;
      else
        assert(false &&
      ...
      // expression op=(PLUS | MINUS) expression
                   ...
                   expr)) {
      if (e->PLUS())
        n->kind = ExprNode::ExprKind::Add;
      else if (e->MINUS())
        n->kind = ExprNode::ExprKind::Sub;
      else
      ...
      // expression op=BitshiftOperator expression
                   ...
                   expr)) {
      if (e->op->getText() == "<<")
        n->kind = ExprNode::ExprKind::LShift;
      else if (e->op->getText() == ">>")
        n->kind = ExprNode::ExprKind::RShift;
      else
      ...
      // expression op=ComparisonOperator expression
                   ...
                   dynamic_cast<qasm3Parser::ComparisonExpressionContext*>(
                       expr)) {
      if (e->op->getText() == ">")
        n->kind = ExprNode::ExprKind::Gt;
      else if (e->op->getText() == "<")
        n->kind = ExprNode::ExprKind::Lt;
      else if (e->op->getText() == ">=")
        n->kind = ExprNode::ExprKind::Ge;
      else if (e->op->getText() == "<=")
        n->kind = ExprNode::ExprKind::Le;
      else
      ...
      // expression op=EqualityOperator expression
                   ...
                   expr)) {
      if (e->op->getText() == "==")
        n->kind = ExprNode::ExprKind::Eq;
      else if (e->op->getText() == "!=")
        n->kind = ExprNode::ExprKind::Ne;
      else
      ...
      if (e->DecimalIntegerLiteral()) {
        ...
        if (pit != param_idx.end()) {
          ...
          if (cit != const_vals.end()) {
            ...
            addError(errUndefined(UndefinedKind::Variable, id), expr);
        }
      } else {
  ...
  C evalC(qasm3Parser::ExpressionContext* expr) {
    ...
    if (auto* e = dynamic_cast<qasm3Parser::CallExpressionContext*>(expr)) {
      if (e->expressionList() && !e->expressionList()->expression().empty()) {
