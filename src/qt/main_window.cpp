...
MainWindow::MainWindow(QWidget* parent) : QMainWindow{parent} {
 ...
 splitter_->setStyleSheet("QSplitter::handle { background: #3a3a3a; height: 4px; }");
 ...
 connect(editor_,      &TextEditor::modifiedChanged,
         this,         &MainWindow::onModifiedChanged);
 connect(editor_,      &TextEditor::fileChanged,
         this,         [this](const QString&) { updateTitle(); });
 connect(controller_,  &AppController::parseSuccess,
         this,         &MainWindow::onParseSuccess);
 connect(controller_,  &AppController::parseError,
         this,         &MainWindow::onParseFail);
...
void MainWindow::setupMenuBar() {
 ...
 fileMenu->setStyleSheet("QMenu { background: #2d2d2d; color: #ddd; }"
 ...
 auto* saveAsAct= fileMenu->addAction("Save &As…", this, &MainWindow::saveFileAs);
 fileMenu->addSeparator();
 fileMenu->addAction("&Quit", this, &QWidget::close, QKeySequence::Quit);

 Q_UNUSED(newAct) Q_UNUSED(openAct) Q_UNUSED(saveAct) Q_UNUSED(saveAsAct)
 ...
 connect(splitAct, &QAction::triggered, this, [this] {
  splitter_->setOrientation(
   splitter_->orientation() == Qt::Vertical ? Qt::Horizontal : Qt::Vertical);
 });
...
void MainWindow::openFile() {
 QString path = QFileDialog::getOpenFileName(this, "Open QASM File", {},
                                             "QASM Files (*.qasm *.qasm2);;All Files (*)");
...
void MainWindow::onParseSuccess() {
 ...
 statusLabel_->setStyleSheet("color: #4CAF50; padding: 0 6px;"); // Green color for success
}

void MainWindow::onParseFail(const QStringList& errors) {
 statusLabel_->setText(QString("Syntax Error: %1 errors").arg(errors.size()));
 statusLabel_->setStyleSheet("color: #F44336; padding: 0 6px;"); // Red color for failure
}
