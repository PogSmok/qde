...
Operation gateOp(const GateRegistry& reg, const std::string& name,
  ...
  return {OperationType::kGate, reg.find(name), params, qubits, {}};
}
...
Operation resetOp(std::vector<QubitReference> qubits) {
  return {OperationType::kReset, nullptr, {}, qubits, {}};
}
}  // namespace
...
TEST_F(SimulatorTest, InitialStateDensityMatrixHasSingleOne) {
  auto states = sim_.run(SimulationCircuit(makeCircuit({{"q", 1}}, {}, {})));
  ...
  for (std::size_t bg = 0; bg < d; ++bg) {
    ...
    for (std::size_t a = 0; a < 2; ++a)
      for (std::size_t b = 0; b < 2; ++b)
        out[a * 2 + b] += rho[(bg | (a << qubit)) * d + (bg | (b << qubit))];
  }
  return out;
}
